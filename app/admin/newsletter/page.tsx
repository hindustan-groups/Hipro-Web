  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="bg-white border border-slate-200 rounded-none-none p-5 flex items-center gap-4 shadow-sm hover:shadow-md transition-shadow">
          <div className="w-10 h-10 rounded-none-none bg-gradient-to-br from-construction-navy to-blue-700 flex items-center justify-center shadow-sm">
        ...
        <div className="bg-white border border-slate-200 rounded-none-none p-5 flex items-center gap-4 shadow-sm hover:shadow-md transition-shadow">
          <div className="w-10 h-10 rounded-none-none bg-gradient-to-br from-construction-red to-red-700 flex items-center justify-center shadow-sm">
      ...
      {error && (
        <div className="p-4 rounded-none-none bg-red-50 border border-red-100 text-red-600 text-sm">{error}</div>
      )}

      {/* Table */}
      <div className="bg-white border border-slate-200 rounded-none-none overflow-hidden shadow-sm">
                  ...
                  <tr key={i} className="animate-pulse">
                    {[...Array(5)].map((_, j) => (
                      <td key={j} className="px-5 py-4">
                        <div className="h-4 bg-slate-100 rounded-none-none" />
                      </td>
                    ))}
                  </tr>
                ))
                ...
                filtered.map((s, i) => (
                  <tr key={s.id} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className="px-5 py-4">
                      <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-none-full text-[11px] font-bold bg-green-100 text-green-700">
                        <span className="w-1.5 h-1.5 rounded-none-full bg-green-500" />
                        Active
                      </span>
                    </td>
                    <td className="px-5 py-4">
                      <button
                        ...
                        className="w-8 h-8 rounded-none-none bg-white border border-slate-200 hover:bg-red-50 hover:border-red-200 text-slate-400 hover:text-red-600 flex items-center justify-center transition-colors shadow-sm"
                        title="Unsubscribe"
                      >
