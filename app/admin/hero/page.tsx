    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        ...
        <div className="flex gap-3">
          ...
          <button onClick={() => setIsAdding(!isAdding)}
            className="flex items-center gap-2 bg-construction-navy hover:bg-blue-800 text-white px-4 py-2 rounded-xl text-sm font-bold transition-colors shadow-sm shadow-blue-900/20">
            <Plus className="w-4 h-4" /> Add Slide
          </button>
        </div>
      </div>

      {error && <div className="p-4 rounded-xl bg-red-50 border border-red-100 text-red-600 text-sm font-medium">{error}</div>}
        ...
        <div className="bg-white border border-slate-200 shadow-sm rounded-2xl p-6 mb-6">
            ...
            <div className="space-y-4">
              <div>
                ...
                <input type="text" value={newSlide.tagline} onChange={e => setNewSlide({...newSlide, tagline: e.target.value})}
                  className="w-full bg-slate-50 border border-slate-200 text-slate-900 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-construction-navy/20"
              ...
              <div>
                ...
                <textarea value={newSlide.subtitle} onChange={e => setNewSlide({...newSlide, subtitle: e.target.value})} rows={3}
                  className="w-full bg-slate-50 border border-slate-200 text-slate-900 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-construction-navy/20 resize-none"
          ...
          <div className="flex gap-3 mt-6">
            <button onClick={handleSaveNew} disabled={saving}
              className="bg-construction-navy hover:bg-blue-800 text-white px-6 py-2.5 rounded-xl text-sm font-bold transition-colors shadow-sm disabled:opacity-50">
              {saving ? "Saving..." : "Save Slide"}
            </button>
            <button onClick={() => setIsAdding(false)} className="px-6 py-2.5 rounded-xl text-slate-500 hover:bg-slate-50 text-sm font-semibold transition-colors">
              Cancel
            </button>
          </div>
        </div>
      ...
      {loading ? (
        <div className="space-y-4 animate-pulse">
          {[1,2].map(i => <div key={i} className="h-32 bg-white border border-slate-200 shadow-sm rounded-2xl" />)}
        </div>
      ) : slides.length === 0 ? (
        ...
        <div className="space-y-4">
          {slides.map((slide) => (
            <div key={slide.id} className="bg-white border border-slate-200 shadow-sm rounded-2xl p-6 transition-shadow hover:shadow-md">
              {editingId === slide.id ? (
                // Edit Mode
                <div className="grid md:grid-cols-2 gap-6">
                  <div className="space-y-4">
                    ...
                    <div className="flex gap-4 pt-2">
                      <label className="flex items-center gap-2 text-sm font-medium text-slate-700 cursor-pointer">
                        <input type="checkbox" defaultChecked={slide.active} id={`active-${slide.id}`} className="rounded border-slate-300 text-construction-navy focus:ring-construction-navy" />
                        Active Slide
                      </label>
                      <div className="flex items-center gap-2 text-sm font-medium text-slate-700">
                        <span>Order:</span>
                        <input type="number" defaultValue={slide.order} id={`order-${slide.id}`} className="w-16 px-2 py-1 bg-slate-50 border border-slate-200 rounded-lg text-center" />
                      </div>
                    </div>
                  </div>
                  <div className="space-y-4">
                    ...
                    <div className="flex gap-3 justify-end mt-4">
                      <button onClick={() => setEditingId(null)} className="px-5 py-2 rounded-xl text-slate-500 hover:bg-slate-50 text-sm font-semibold transition-colors">
                        Cancel
                      </button>
                      <button onClick={() => {
                        ...
                        className="bg-construction-navy hover:bg-blue-800 text-white px-6 py-2 rounded-xl text-sm font-bold transition-colors shadow-sm disabled:opacity-50">
                ...
                <div className="flex flex-col md:flex-row gap-6">
                  ...
                  <div className="flex-1 flex gap-4">
                    ...
                    <div className="flex-1 space-y-2">
                      <div className="flex items-center gap-3">
                        <span className="text-[10px] font-bold uppercase tracking-widest text-construction-navy bg-blue-50 px-2 py-0.5 rounded-md border border-blue-100">
                          {slide.tagline}
                        </span>
                        {!slide.active && (
                          <span className="flex items-center gap-1 text-[10px] font-bold uppercase tracking-widest text-amber-600 bg-amber-50 px-2 py-0.5 rounded-md border border-amber-200">
                  ...
                  <div className="flex items-center justify-between md:justify-end gap-6 md:w-64 shrink-0">
                    <div className="w-24 h-16 rounded-lg border border-slate-200 overflow-hidden bg-slate-100 relative">
                      {slide.image && (
                    ...
                    <div className="flex items-center gap-2">
                      <button onClick={() => setEditingId(slide.id!)}
                        className="px-3 py-1.5 text-xs font-bold text-slate-600 hover:text-construction-navy bg-slate-50 hover:bg-blue-50 border border-slate-200 hover:border-blue-200 rounded-lg transition-colors">
                        Edit
                      </button>
                      <button onClick={() => handleDelete(slide.id!)}
                        className="px-3 py-1.5 text-xs font-bold text-red-600 hover:text-white bg-red-50 hover:bg-red-600 border border-red-200 hover:border-red-600 rounded-lg transition-colors">
                        Delete
                      </button>
                    </div>
                  </div>
