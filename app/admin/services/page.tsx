                ...
                {loading ? (
                  ...
                  services.map((s) => (
                    <tr key={s.id} className={`transition-colors ${s.active === false ? "opacity-50 bg-slate-50/50" : "hover:bg-slate-50"}`}>
                      ...
                      <td className="px-5 py-4"><button onClick={() => toggleActive("services", s)} className={`w-8 h-8 rounded-none-none flex items-center justify-center ${s.active !== false ? "bg-green-100 text-green-700" : "bg-slate-50 text-slate-400"}`}>{s.active !== false ? <Eye className="w-4 h-4" /> : <EyeOff className="w-4 h-4" />}</button></td>
                      <td className="px-5 py-4"><button onClick={() => deleteItem("services", s.id as string)} className="w-8 h-8 rounded-none-none text-slate-400 hover:text-red-600"><Trash2 className="w-4 h-4" /></button></td>
                    </tr>
                  ))
                )}
              </tbody>
