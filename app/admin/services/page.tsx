...
const EMPTY_SERVICE: Omit<Service, "id" | "createdAt"> = {
  title: "", description: "", icon: "Wrench", image: "", order: 99, active: true, features: []
};
...
export default function AdminServices() {
  ...
  const handleServiceSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res  = await fetch("/api/services", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(serviceForm) });
      const json = await res.json();
      if (json.success) { setShowForm(false); setServiceForm(EMPTY_SERVICE); fetchData(); }
      ...
      {showForm && activeTab === "services" && (
        <form onSubmit={handleServiceSubmit} className="bg-white border border-slate-200 shadow-sm rounded-none p-6 space-y-4">
          ...
          <div className="flex gap-3 pt-2">
            <button type="submit" disabled={saving} className="bg-construction-navy hover:bg-blue-800 text-white px-6 py-2 rounded-none text-sm font-semibold disabled:opacity-50">Save</button>
            <button type="button" onClick={() => setShowForm(false)} className="bg-white border border-slate-200 text-slate-600 px-6 py-2 rounded-none text-sm font-semibold">Cancel</button>
          </div>
        </form>
      )}

      {showForm && activeTab === "guarantees" && (
        <form onSubmit={handleGuaranteeSubmit} className="bg-white border border-slate-200 shadow-sm rounded-none p-6 space-y-4">
          ...
          {activeTab === "services" ? (
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-slate-100">
                {loading ? (
                  [...Array(3)].map((_, i) => <tr key={i} className="animate-pulse"><td colSpan={5} className="px-5 py-4"><div className="h-10 bg-slate-100 rounded-none" /></td></tr>)
                ) : services.length === 0 ? (
                  <tr><td colSpan={5} className="text-center text-slate-500 py-16">No services yet.</td></tr>
                ) : (
                  services.map((s) => (
                ...
                {loading ? (
                  ...
                  guarantees.map((g) => (
                    <tr key={g.id} className={`transition-colors ${g.active === false ? "opacity-50 bg-slate-50/50" : "hover:bg-slate-50"}`}>
                      ...
                      <td className="px-5 py-4"><span className="text-xs bg-slate-100 px-2 py-1 rounded-none">{g.badge}</span></td>
                      <td className="px-5 py-4 text-slate-600 whitespace-nowrap">{g.order}</td>
                      <td className="px-5 py-4"><button onClick={() => toggleActive("guarantees", g)} className={`w-8 h-8 rounded-none flex items-center justify-center ${g.active !== false ? "bg-green-100 text-green-700" : "bg-slate-50 text-slate-400"}`}>{g.active !== false ? <Eye className="w-4 h-4" /> : <EyeOff className="w-4 h-4" />}</button></td>
                      <td className="px-5 py-4"><button onClick={() => deleteItem("guarantees", g.id as string)} className="w-8 h-8 rounded-none text-slate-400 hover:text-red-600"><Trash2 className="w-4 h-4" /></button></td>
                    </tr>
                  ))
                )}
              </tbody>
