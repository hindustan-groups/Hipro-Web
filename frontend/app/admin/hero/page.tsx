    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        ...
        <div className="flex gap-3">
          ...
          <button onClick={() => { setIsAdding(!isAdding); setEditingSlide(null); }}
              ...
              {editingSlide?.id === slide.id ? (
                ...
                <div className="grid md:grid-cols-2 gap-6">
                  <div className="space-y-4">
                    <div>
                      <label className="text-slate-500 text-[10px] uppercase tracking-widest font-bold block mb-1">Tagline</label>
                      <input type="text" value={editingSlide.tagline || ""} onChange={e => setEditingSlide({...editingSlide, tagline: e.target.value})}
                        className="w-full bg-slate-50 border border-slate-200 text-slate-900 rounded-none px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-construction-navy/20" />
                    </div>
                    <div>
                    ...
                    <div>
                      <label className="text-slate-500 text-[10px] uppercase tracking-widest font-bold block mb-1">Subtitle</label>
                      <textarea value={editingSlide.subtitle || ""} onChange={e => setEditingSlide({...editingSlide, subtitle: e.target.value})} rows={3}
                        className="w-full bg-slate-50 border border-slate-200 text-slate-900 rounded-none px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-construction-navy/20 resize-none" />
                    </div>
                    <div className="flex gap-4 pt-2">
                      <label className="flex items-center gap-2 text-sm font-medium text-slate-700 cursor-pointer">
                        <input type="checkbox" checked={editingSlide.active ?? true} onChange={e => setEditingSlide({...editingSlide, active: e.target.checked})} className="border-slate-300 text-construction-navy focus:ring-construction-navy" />
                        Active Slide
                      </label>
                      <div className="flex items-center gap-2 text-sm font-medium text-slate-700">
                        <span>Order:</span>
                        <input type="number" value={editingSlide.order || 0} onChange={e => setEditingSlide({...editingSlide, order: parseInt(e.target.value) || 0})} className="w-16 px-2 py-1 bg-slate-50 border border-slate-200 rounded-none text-center" />
                      </div>
                    </div>
                  </div>
                  <div className="space-y-4">
                    ...
                    <div className="bg-slate-50 border border-slate-200 rounded-none p-4">
                      <ImageUpload
                        ...
                        onChange={(url) => setEditingSlide(prev => prev ? { ...prev, image: url } : prev)}
                      />
                    </div>
                    <div className="flex gap-3 justify-end mt-4">
                      <button onClick={() => setEditingSlide(null)} className="px-5 py-2 rounded-none text-slate-500 hover:bg-slate-50 text-sm font-semibold transition-colors">
                        Cancel
                      </button>
                      <button onClick={() => {
                        if (editingSlide.id) {
                          handleUpdate(editingSlide.id, {
                            tagline: editingSlide.tagline,
                ...
                <div className="flex flex-col md:flex-row gap-6">
                  ...
                  <div className="flex items-center justify-between md:justify-end gap-6 md:w-64 shrink-0">
                    ...
                    <div className="flex items-center gap-2">
                      <button onClick={() => { setEditingSlide({ ...slide }); setIsAdding(false); }}
