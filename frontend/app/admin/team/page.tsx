    ...
    <div className="space-y-5">
      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="bg-white border border-slate-200 shadow-sm rounded-none-none p-6 space-y-4">
          ...
          <div className="flex items-center gap-2 p-4 bg-amber-50/50 border border-amber-200 rounded-none-none">
            <input 
              type="checkbox" 
              id="isFounder"
            ...
            {(["instagram", "linkedin", "facebook"] as const).map((field) => (
              <div key={field}>
                ...
                <input value={form[field] || ""} onChange={(e) => setForm((p) => ({ ...p, [field]: e.target.value }))} placeholder="https://..."
                  className="w-full bg-slate-50 border border-slate-200 text-slate-900 rounded-none-none px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-construction-navy/20 focus:border-construction-navy transition-all" />
              </div>
            ))}
          </div>
          ...
          <div>
            ...
            <textarea rows={4} value={form.bio} onChange={(e) => setForm((p) => ({ ...p, bio: e.target.value }))}
              className="w-full bg-slate-50 border border-slate-200 text-slate-900 rounded-none-none px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-construction-navy/20 focus:border-construction-navy transition-all resize-none" />
          </div>
                ...
                team.map((m) => (
                  <tr key={m.id} className={`transition-colors ${m.active === false ? "opacity-50 bg-slate-50/50" : "hover:bg-slate-50"}`}>
                    <td className="px-5 py-4">
                      {m.img ? <img src={m.img} alt={m.name} className="w-10 h-10 rounded-none-full object-cover border border-slate-200" /> : <div className="w-10 h-10 rounded-none-full bg-slate-200" />}
                    </td>
                    <td className="px-5 py-4">
                      <p className="text-slate-900 font-semibold flex items-center gap-2">
                        {m.name}
                        {m.isFounder && <span className="text-[9px] font-bold uppercase tracking-wider bg-amber-100 text-amber-700 px-1.5 py-0.5 rounded-none-none border border-amber-200">Founder</span>}
                      </p>
                    ...
                    <td className="px-5 py-4">
                      <button onClick={() => toggleActive(m)} className={`w-8 h-8 rounded-none-none flex items-center justify-center transition-colors ${m.active !== false ? "bg-green-100 text-green-700 border border-green-200" : "bg-slate-50 border border-slate-200 text-slate-400 hover:text-slate-700"}`}>
                    ...
                    <td className="px-5 py-4">
                      <div className="flex items-center gap-2">
                        <button onClick={() => handleEdit(m)}
                          className="flex items-center gap-1.5 px-3 py-1.5 rounded-none-none bg-white border border-slate-200 hover:bg-slate-50 hover:text-construction-navy hover:border-slate-300 text-slate-600 font-semibold transition-all shadow-sm">
